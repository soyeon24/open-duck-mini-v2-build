...
| ☐ | `head_yaw_to_roll.stl` | **x1** | 88.0 × 80.0 × 42.2 | PLA |
| ☐ | `head_pitch_to_yaw.stl` | **x1** | 32.0 × 44.9 × 72.0 | PLA |
...
- **안테나(귀)는 이 목록에 없음** — 시뮬 메시엔 `antenna.stl`(길이 146mm)이 있지만 print_guide엔 빠져 있음. 봉재 구매 항목인지 BOM에서 확인 필요
- M3 열삽입 인서트 · 608ZZ 베어링은 별도 구매 (NEXT_STEPS.md 하드웨어 스펙 참조)