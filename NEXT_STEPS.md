...
## 결정 사항
- **목표**: Microduck 대신 **Open Duck Mini v2**를 직접 제작 (Microduck 은 BOM·배선도·조립도가 없고
  커스텀 HAT PCB 의 schematic/gerber 가 비공개라 자작 불가, $399 완제품 예약판매만 가능)
  - ⚠️ 2026-09-10 정정: "Microduck HW 전부 비공개"는 틀렸다. `microduck_rl` 의
    `src/mjlab_microduck/robot/microduck/assets/` 에 STL 43종 + MJCF 풀셋이 Apache-2.0 으로 공개돼 있고,
    컴퓨트 보드도 커스텀이 아니라 시판 **Radxa Zero 3W**(RK3566) 다. 막히는 건 커스텀 HAT 과 조립 문서다
...
- 납땜 가능 ✅ / 3D 프린터 접근 가능 ✅ — **Bambu Lab H2D 또는 X1 Carbon** (2026-09-01 확인)
  - ⚠️ **다른 랩실 프린터**라 며칠 점유가 부담 → 외주 전면 검토했고 **2026-09-10 결론 남**

### 출력 방식 확정 (2026-09-10)

| | 물량 | 비용 / 시간 |
|---|---|---|
| **아이컨택(국내)** | `foot_bottom_tpu` ×2 (TPU) | **33,600원** (배송·부가세 포함) |
| **랩실 H2D** | PLA 49개 | **4판 / 약 16시간** |
| 필라멘트 | PLA 1kg × 2롤 | 4~6만원 |
| | | **합계 약 7~9만원** |

부품 상세는 `print/OpenDuckMiniV2_parts.xlsx` (36종 ABC순, 치수·체적·필라멘트·M3 인서트·업체별 최소치수 통과 여부).

**외주 탈락 사유 — 다시 헤매지 말 것**
- JLC3DP **SLA**($145, 최저가): M3 열삽입 인서트 불가. 레진은 열경화성이라 안 녹음 → 조립 불가
- JLC3DP **MJF**($600): 인필 없이 솔리드라 출력물 990g→1,607g, 로봇 전체 +29% → MJCF 질량과 어긋나 정책 재학습 필요
- JLC3DP **FDM ABS**($455): 수축률 0.5~0.8%로 head(200mm) 휨
- JLC3DP **FDM PLA-P**($143 + 통관 15~20% ≈ 24만원): 소재는 맞지만 **22개만 통과**, 29개는 최소치수(30×30×10) 미달
- **meviy**: 탄성 소재 자체가 없음(POM·MC나일론·ABS·PTFE·PP·PEEK 전부 절삭용 경질재). 절삭으로는 고무를 못 깎음
- **UOS 3D창작터**: 학생 사용 불가
- **근본 원인**: 이 설계는 두께 15mm 미만 얇은 판재가 29개(57%). FDM 프린터엔 가장 쉬운 형상인데 출력 대행 업체는 파손·적재 문제로 최소치수를 걸어 거부함. **설계와 외주의 궁합이 나쁨**

**TPU 발바닥**: 어느 업체도 안 받았으나(JLC TPU Min 9×9×30에 두께 8.0mm가 1mm 미달) 아이컨택만 통과.
사서 쓰면 랩실 프린터에 TPU를 넣을 필요가 없어짐 — 허가 문제와 첫 TPU 출력 리스크가 함께 사라짐.
(랩실에서 뽑을 경우 대비 메모: Bambu 전 기종 다이렉트 드라이브라 가능하지만 **AMS 금지, 외부 스풀 홀더**로 저속)

**슬라이서 설정**: 0.4mm 노즐 / 0.20mm 레이어 / 인필 15% / 서포트 자동 — 전부 기본값.
36종 최소 살두께가 1.26mm라 0.4 노즐로 문제없음. `trunk_top`의 Ø2.0 구멍 때문에 0.6 노즐로 바꾸지 말 것
...
- M3 열삽입 인서트, **626ZZ(6×19×6)** 베어링 ×3 — ⚠️ 608ZZ 아님 (STL 포켓 실측 Ø19.8~19.9, `HARDWARE_PREP.md` §4)
